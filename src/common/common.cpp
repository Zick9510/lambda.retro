...
void print_ast(Arena& arena, NodeId node_id, uint32_t depth) {
  ...
  if (auto* g = std::get_if<Global>(&node.data)) {
    ...
    print_ast(arena, f->body, depth + 1);
    std::cout << ')';

  } else if (auto* a = std::get_if<App>(&node.data)) {
    std::cout << "( ";
    print_ast(arena, a->func, depth);
    std::cout << ' ';
    print_ast(arena, a->arg , depth);
    std::cout << ')';
